...
function handler (req, res) {
  ...
  if (req.url == "/control") { file = "/control.html"; }
  if (req.url == "/tweetscreen.png") { file = "/tweetscreen.png"; type = "image/png"; }
...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('down', function (position) {
    ...
    setTimeout(function () { moveZ(downZ); }, 100);
  });
...
calculateTranslation = function () {
  var b0x = botPlane[0].x, b0y = botPlane[0].y, b1x = botPlane[1].x, b1y = botPlane[1].y;
  var d0x = devicePlane[0].x, d0y = devicePlane[0].y, d1x = devicePlane[1].x, d1y = devicePlane[1].y;

  var M = $M([[d0x, d0y, 1, 0],[-d0y, d0x, 0, 1],[d1x, d1y, 1, 0],[-d1y, d1x, 0, 1]]);
  var u = $M([[b0x],[b0y],[b1x],[b1y]]);
  var MI = M.inverse();
...
calibrate = function () {
  ...
  function doIt () {
    ...
    moveDownUntilTap(function (err) {}, function (tap) {
      ...
      botPlane.push({x: position()[1], y: position()[2], z: position()[3]});
      downZ = position()[3];
      ...
      if (iter <= 2) {
        ...
        if (iter == 2) { x -= 30; y -= 35; }
        doIt();
        if (iter == 2) { calculateTranslation(); }
      }
    });
  }
